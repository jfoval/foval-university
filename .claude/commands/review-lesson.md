...
Launch four subagents in parallel, each with fresh context. Give each: the full path to the lesson file, the path to `docs/EDITORIAL_STANDARDS.md`, the path to `<course>/research/SOURCES.md`, and the instruction to read all three before starting. Their briefs:

1. **Depth reviewer**: "Review against Part 1 of the standards. Apply the specificity, expert, and transfer tests to every section. Quote every passage that is generic, hedged, or could have been written without knowing the subject. Check every required element in 1.2 is present and substantive. Be harsh; the goal is to find weaknesses."
